            ...
            if event.to_me:
                ...
                if temp == 2:
                    ...
                    write_msg(event.user_id,
                              f"Данные успешно обновлены {event.user_id}, {instituteName}, {semestrNumber}, {lessonName}",
                              start_key)
                    return array[i - 1]
    ...
    if user:
        ...
        if check == None:
            user = colUsers.update_one({"vkid": vkId}, {'$push': {
                "lessons": {"institutename": instituteName, "semestr": semestrNumber, "lessonname": lessonName}}})
    else:
        colUsers.insert_one({"vkid": vkId, "username": userName, "status": True, "lessons": [
            {"institutename": instituteName, "semestr": semestrNumber, "lessonname": lessonName}]})
...
def getUserLessons(vkId, colUsers):
    ...
    if user:
        ...
        for lesson in user["lessons"]:
            message = message + str(i) + ")" + str(lesson["semestr"]) + ' семестр ' + lesson["institutename"] + ' ' + \
    ...
    for event in longpoll.listen():
        if event.type == VkEventType.MESSAGE_NEW:
            if event.to_me:
                ...
                if (step == 4 and status == 0) or (add_lesson == 3) or (step == 4 and status == 1 and temp_tst == 2):
